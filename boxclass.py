class MovingBox:
    ...
    def add_item(self, item_name, item_count):
        self.items[item_name] = self.items.get(item_name, 0) + item_count
    ...
    def transfer_item(self, target, item_name, item_count):
        results = self.remove_item(item_name, item_count)
    ...
    def __repr__(self):
        content = [f'Box {self.name} contains:']
        for item_name, item_count in self.items.items():
            content.append(f'\t{item_name}: x{item_count:3}')
        return '\n'.join(content)
